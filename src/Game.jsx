  ...
  useEffect(()=>{
    const down=e=>{
      ...
      if(e.key==="z"||e.key==="Z"){
        ...
        else if(dDL<INTERACT_DIST+10)setGamePhase(safeDoor==="left"?"win":"lose");
        else if(dDR<INTERACT_DIST+10)setGamePhase(safeDoor==="right"?"win":"lose");
      }
    };
...
PERSONALITY: Gruff medieval dungeon guard. Short sentences, 2-3 max. Menacing but willing to answer. Never break character or explain the puzzle.`;
  ...
  if(gamePhase==="win") return (
    <div style={{fontFamily:FONT,background:"#0a1a0a",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",padding:20}}>
      {muteBtn}
      <div style={{fontSize:28,color:"#70ff70",textShadow:"0 0 30px #70ff70, 0 0 60px #40aa40",marginBottom:20,animation:"pulse 1.5s infinite"}}>✦ FREEDOM ✦</div>
      <div style={{fontSize:9,color:"#a0e8a0",lineHeight:"22px",maxWidth:400,marginBottom:10}}>The door swings open to reveal a sun-drenched meadow. But your adventure is far from over...</div>
      <div style={{fontSize:40,margin:"10px 0"}}>🌅🌿🕊️</div>
      <div style={{display:"flex",gap:12,marginTop:20}}>
        <button onClick={onWin} style={{fontFamily:FONT,fontSize:10,padding:"12px 28px",background:"#70ff70",color:"#0a1a0a",border:"none",borderRadius:6,cursor:"pointer",boxShadow:"0 0 20px #70ff70"}}>NEXT LEVEL →</button>
        <button onClick={onRestart} style={{fontFamily:FONT,fontSize:9,padding:"10px 20px",background:"transparent",color:"#70aa70",border:"2px solid #70aa70",borderRadius:6,cursor:"pointer"}}>RESTART</button>
      </div>
    ...
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",fontFamily:FONT,background:"#0e0c14",minHeight:"100vh",padding:"12px 8px",boxSizing:"border-box"}} onClick={startMusicForLevel}>
      ...
      {chatOpen&&(
        <div style={{width:L1_W+12,maxWidth:"100%",background:"#0c0a16",border:"3px solid "+(talkingTo==="left"?"#b04040":"#4060b0"),borderRadius:6,marginTop:6,padding:10,boxSizing:"border-box"}}>
          ...
          {sayingBye?
            <button ref={walkAwayRef} onClick={()=>{setSayingBye(false);setChatOpen(false);}} onKeyDown={e=>{e.stopPropagation();if(e.key==="Enter"||e.key===" "||e.key==="Escape"||e.key==="z"||e.key==="Z"){e.preventDefault();setSayingBye(false);setChatOpen(false);}}} style={{marginTop:6,background:"transparent",border:"1px solid #3a3060",borderRadius:4,color:"#e8d070",fontFamily:FONT,fontSize:7,padding:"4px 8px",cursor:"pointer",width:"100%"}}>DISMISS</button>
          :
            <button ref={walkAwayRef} onClick={()=>{const g=talkingTo==="left"?"Hmph. Don't take too long, prisoner.":"Watch your step out there...";if(talkingTo==="left")setMessagesL(p=>[...p,{role:"assistant",text:g}]);else setMessagesR(p=>[...p,{role:"assistant",text:g}]);setSayingBye(true);}} onKeyDown={e=>{e.stopPropagation();if(e.key==="Enter"||e.key===" "){e.preventDefault();const g=talkingTo==="left"?"Hmph. Don't take too long, prisoner.":"Watch your step out there...";if(talkingTo==="left")setMessagesL(p=>[...p,{role:"assistant",text:g}]);else setMessagesR(p=>[...p,{role:"assistant",text:g}]);setSayingBye(true);}if(e.key==="Escape"){e.preventDefault();setChatOpen(false);}if(e.key==="ArrowUp"){e.preventDefault();inputRef.current?.focus();}}} style={{marginTop:6,background:"transparent",border:"1px solid #3a3060",borderRadius:4,color:"#6a6a8a",fontFamily:FONT,fontSize:7,padding:"4px 8px",cursor:"pointer",width:"100%"}}>THANKS, GOODBYE</button>
          }
        </div>
      )}
    </div>
...
const L2_ALL_NPCS=[
  ...
  {key:"lead",room:"hall",x:14*TILE,y:10*TILE,label:"LEAD DETECTIVE",color:"#b89868",draw:drawLeadDetective},
  ...
  {key:"viscount",room:"bathroom",x:8*TILE,y:6*TILE,label:"VISCOUNT",color:"#6b3a8b",draw:drawViscountEminence},
...
function drawEntryHall(ctx) {
  ...
  // Bottom wall
  for(let c=0;c<L2_COLS;c++){ctx.fillStyle="#6b5040";ctx.fillRect(c*TILE,(L2_ROWS-1)*TILE,TILE,TILE);ctx.fillStyle="#5a4030";ctx.fillRect(c*TILE+2,(L2_ROWS-1)*TILE+2,TILE-4,TILE-4);}
...
const L2_EXAMINABLES=[
  {key:"painting1",x:6*TILE,y:1*TILE,room:"hall",label:"PAINTING - Red Abstract",drawCloseup:drawCloseupPainting1,description:"A bold abstract painting in crimson. The heavy gilded frame could certainly do damage. Wait — there's a thread caught on the frame... it's mauve-colored, matching VP Mauve's outfit."},
  ...
  {key:"frame",x:3*TILE,y:3*TILE,room:"hall",label:"EVIDENCE - Heavy Painting Frame",drawCloseup:drawCloseupFrame,description:"An antique painting with an unusually heavy iron frame. Could deliver a lethal blow."},
  {key:"statue",x:4*TILE,y:3*TILE,room:"hall",label:"EVIDENCE - Abstract Statue",drawCloseup:drawCloseupStatue,description:"An abstract stone statue. Dense, heavy, with sharp angular edges. There appear to be faint marks on the base."},
...
function Level2({ onWin, onRestart, muted, setMuted, muteBtn, startMusicForLevel, apiKey, model }) {
  ...
  const keysRef=useRef({});
  const playerRef=useRef({x:10*TILE,y:10*TILE,dir:"up",frame:0,tick:0});
  ...
  const leadPosRef=useRef({x:14*TILE, y:4*TILE});
  ...
  const wanderRef=useRef({
    ...
    viscount:{x:8*TILE,y:6*TILE,homeX:8*TILE,homeY:6*TILE,tx:8*TILE,ty:6*TILE,timer:0,frame:0,tick:0,dir:"down"},
        ...
        for(const npc of roomNpcs){
          ...
          if(isFacing(nx,ny)){
            ...
            setMessages(prev=>{
              ...
              if(npc.key==="soothsayer")greeting="The spirits whisper to me... One of the three suspects speaks only lies. But the mists obscure which one... Trust your wits, detective.";
        ...
        if(!foundNpc){
          ...
          for(const ex of roomExams){
            if(isFacing(ex.x,ex.y)){
              setExamining(ex);
              if(ex.key==="painting1") setDiscoveredClues(prev => prev.includes("A mauve thread was found on the painting — VP Mauve had the painting.") ? prev : [...prev, "A mauve thread was found on the painting — VP Mauve had the painting."]);
  ...
  useEffect(()=>{
    ...
    const loop=()=>{
      ...
      // Cutscene: Lead Detective walks toward player (cardinal only, avoids caution tape)
      if(cutsceneRef.current&&curRoom==="hall"){
        const lp=leadPosRef.current;
        const dx=p.x-lp.x,dy=p.y-lp.y;
        const dist=Math.hypot(dx,dy);
        // Caution tape bounds
        const CTX=8*TILE-4,CTY=5*TILE+16,CTW=4*TILE+8,CTH=2*TILE+16;
        const inTape=(tx,ty)=>tx+TILE>CTX&&tx<CTX+CTW&&ty+TILE>CTY&&ty<CTY+CTH;
        if(dist>TILE*1.5){
          const spd=1.8;
          // Move one axis at a time (cardinal only)
          let mx=0,my=0;
          if(Math.abs(dx)>Math.abs(dy)){mx=dx>0?spd:-spd;}
          else{my=dy>0?spd:-spd;}
          const nextX=lp.x+mx,nextY=lp.y+my;
          if(!inTape(nextX,nextY)){
            lp.x=nextX;lp.y=nextY;
          }else{
            // Go around: move on the other axis to clear the tape
            if(mx!==0){lp.y+=(lp.y<CTY)?-spd:spd;}
            else{lp.x+=(lp.x<CTX)?-spd:spd;}
          }
        }else{
      ...
      for(const key of ["mauve","viscount","duchess"]){
        ...
        if(!chatOpenRef.current||talkingTo!==key){
          ...
          if(d>2&&!w.idle){
            ...
            if(Math.hypot(nextX-p.x,nextY-p.y)>TILE*1.2){
              w.x=nextX;w.y=nextY;
...
PERSONALITY: Professional, curt, impatient, condescending toward "time-wasters." You check your watch constantly. You deflect personal questions with corporate jargon. You look down on aristocrats but secretly crave their social acceptance.

CORE STATEMENT: When asked about what you saw or know, you always say: "Viscount Eminence brought a rare vase." You can elaborate but never deviate from this substance.
...
PERSONALITY: Haughty, condescending, dramatic. You speak with exaggerated formality and frequently adjust your monocle. You drop subtle insults wrapped in politeness. You are evasive when cornered. Beneath the arrogance is a deeply lonely, bitter man who mourns a life that never was.

CORE STATEMENT: When asked about what you saw or know, you always say: "I noticed the abstract statue was missing from the entry hall. It must have been in one of the other two rooms." You can elaborate but never deviate from this substance.
...
PERSONALITY: Charming, gossipy, theatrical, eccentric. You call everyone "darling" and "dear." You seem harmless and scatterbrained, but occasionally let slip something surprisingly sharp before catching yourself. You are an extraordinary actress — decades of high society have made you a master of masks. You are THE LIAR — your core statement is deliberately misleading to frame the Viscount.

CORE STATEMENT: When asked about what you saw or know, you always say: "The only thing I noticed was Viscount Eminence was in the entry hall around the time of the murder." You can elaborate but never deviate from this substance. This is a LIE designed to cast suspicion on the Viscount.
    ...
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",fontFamily:FONT,background:"#1a1820",minHeight:"100vh",padding:"12px 8px",boxSizing:"border-box"}} onClick={startMusicForLevel}>
      ...
      {chatOpen&&ti&&(
        <div style={{width:L2_W+12,maxWidth:"100%",background:"#1a1420",border:"3px solid "+ti.color,borderRadius:6,marginTop:6,padding:10,boxSizing:"border-box"}}>
          ...
          {sayingBye?
            <button ref={walkAwayRef} onClick={()=>{setSayingBye(false);setChatOpen(false);}} onKeyDown={e=>{e.stopPropagation();if(e.key==="Enter"||e.key===" "||e.key==="Escape"||e.key==="z"||e.key==="Z"){e.preventDefault();setSayingBye(false);setChatOpen(false);}}} style={{marginTop:6,background:"transparent",border:"1px solid #3a3040",borderRadius:4,color:"#e8d070",fontFamily:FONT,fontSize:7,padding:"4px 8px",cursor:"pointer",width:"100%"}}>DISMISS</button>
          :
            <button ref={walkAwayRef} onClick={()=>{const goodbyes={mauve:"Time is money, detective.",viscount:"*waves dismissively* Do come back when you have something useful.",duchess:"Oh, do visit again! This is so exciting!",cop:"Stay sharp, detective. Justice waits for no one.",lead:"Good luck out there. We're counting on you.",soothsayer:"The spirits will be watching...",forensics:"Let me know if you need anything re-examined."};const g=goodbyes[talkingTo]||"Goodbye.";setMessages(prev=>({...prev,[talkingTo]:[...prev[talkingTo],{role:"assistant",text:g}]}));setSayingBye(true);}} onKeyDown={e=>{e.stopPropagation();if(e.key==="Enter"||e.key===" "){e.preventDefault();const goodbyes={mauve:"Time is money, detective.",viscount:"*waves dismissively* Do come back when you have something useful.",duchess:"Oh, do visit again! This is so exciting!",cop:"Stay sharp, detective. Justice waits for no one.",lead:"Good luck out there. We're counting on you.",soothsayer:"The spirits will be watching...",forensics:"Let me know if you need anything re-examined."};const g=goodbyes[talkingTo]||"Goodbye.";setMessages(prev=>({...prev,[talkingTo]:[...prev[talkingTo],{role:"assistant",text:g}]}));setSayingBye(true);}if(e.key==="Escape"){e.preventDefault();setChatOpen(false);}if(e.key==="ArrowUp"){e.preventDefault();inputRef.current?.focus();}}} style={{marginTop:6,background:"transparent",border:"1px solid #3a3040",borderRadius:4,color:"#6a6a8a",fontFamily:FONT,fontSize:7,padding:"4px 8px",cursor:"pointer",width:"100%"}}>THANKS, GOODBYE</button>
          }
        </div>
      )}
...
// ═══════════════════════════════════════
const MODEL_OPTIONS = [
  { id: "llama3.1:8b", label: "Llama 3.1 8B (Free)", provider: "ollama" },
  { id: "llama3.2:3b", label: "Llama 3.2 3B (Faster/Free)", provider: "ollama" },
  { id: "claude-opus-4-20250514", label: "Opus 4", provider: "anthropic" },
  { id: "claude-sonnet-4-20250514", label: "Sonnet 4", provider: "anthropic" },
  { id: "claude-haiku-3-5-20241022", label: "Haiku 3.5", provider: "anthropic" },
];
...
export default function Game() {
  ...
  const [model, setModel] = useState(() => localStorage.getItem("anthropic_model") || MODEL_OPTIONS[0].id);
    ...
    return (
      <div style={{
        ...
        {selectedProvider === "ollama" && (
          <div style={{fontSize:7,color:"#70e870",marginBottom:12,lineHeight:"1.6"}}>✔ Free — runs locally via Ollama. No API key needed.</div>
        )}
        ...
        <div style={{fontSize:7,marginTop:20,color:"#666",maxWidth:350,lineHeight:"1.8"}}>
          {selectedProvider === "anthropic" ? "Your key is stored in localStorage only. Get one at console.anthropic.com" : "Make sure Ollama is running (ollama serve)"}
        </div>
      </div>
    );
  }
