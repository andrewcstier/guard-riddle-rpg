...
function drawZPrompt(ctx, x, y) {
  ...
  ctx.font="14px "+FONT; ctx.textAlign="center"; ctx.fillText("A",x+16,y-6); ctx.textAlign="left";
}
...
function drawDungeon(ctx) {
  ...
  ctx.fillStyle="#e8d070";ctx.font="13px "+FONT;ctx.textAlign="center";ctx.fillText("DOOR 1",DOOR_L.col*TILE+16,TILE*2+14);
...
function Level1({ onWin, onRestart, muted, setMuted, muteBtn, startMusicForLevel, apiKey, model, hardMode }) {
  ...
  useEffect(()=>{if(chatEndRef.current)chatEndRef.current.parentElement.scrollTop=chatEndRef.current.parentElement.scrollHeight;},[messagesL,messagesR]);
  ...
  useEffect(()=>{
    ...
    const loop=()=>{
      ...
      if(!guardsGoneRef.current.left){ctx.globalAlpha=gOp;drawGuard(ctx,gLx,gLy,"left");ctx.font="12px "+FONT;ctx.textAlign="center";ctx.fillStyle="#b04040";ctx.fillText("GUARD 1",gLx+16,gLy-16);ctx.textAlign="left";ctx.globalAlpha=1;}
      if(!guardsGoneRef.current.right){ctx.globalAlpha=gOp;drawGuard(ctx,gRx,gRy,"right");ctx.font="12px "+FONT;ctx.textAlign="center";ctx.fillStyle="#4060b0";ctx.fillText("GUARD 2",gRx+16,gRy-16);ctx.textAlign="left";ctx.globalAlpha=1;}
...
PERSONALITY: Gruff medieval dungeon guard. Short sentences, 2-3 max. Menacing but willing to answer. Never break character or explain the puzzle. NEVER reveal which guard is the liar or truth-teller.`;
  ...
  if(gamePhase==="intro") return (
    <div style={{fontFamily:FONT,background:"#0e0c14",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",padding:20}} onClick={startMusicForLevel}>
      ...
      <button onClick={()=>setGamePhase("play")} style={{fontFamily:FONT,fontSize:10,padding:"14px 28px",background:"#e8d070",color:"#0e0c14",border:"none",borderRadius:6,cursor:"pointer",boxShadow:"0 0 20px rgba(232,208,112,0.4)",marginTop:20,minHeight:48}}>ENTER THE DUNGEON</button><div style={{fontSize:6,marginTop:16,color:"#444"}}>{VERSION}</div>
    </div>
  );
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",fontFamily:FONT,background:"#0e0c14",minHeight:"100vh",padding:"12px 8px 190px",boxSizing:"border-box",position:"relative"}} onClick={startMusicForLevel}>
      {muteBtn}
      <div style={{color:"#e8d070",fontSize:10,marginBottom:8,textShadow:"2px 2px 0 #3a2a0a",letterSpacing:3}}>THE TWO GUARDS</div>
      <div style={{color:"#8a7a6a",fontSize:9,marginBottom:10,textAlign:"center",lineHeight:"16px"}}>One always lies. One always tells the truth. Choose a door.</div>
      ...
      <div style={{color:"#6a6a8a",fontSize:10,marginTop:6,textAlign:"center"}}>
      ...
      {chatOpen&&(<div style={{width:"100%",maxWidth:500,background:"#0c0a16",border:"3px solid "+(talkingTo==="left"?"#b04040":"#4060b0"),borderRadius:6,marginTop:6,padding:10,boxSizing:"border-box"}}>
          ...
          <div style={{maxHeight:120,overflowY:"auto",marginBottom:8,WebkitOverflowScrolling:"touch",cursor:typingText&&typingText.current!==typingText.full?"pointer":"default"}} onClick={()=>{if(typingText&&typingText.current!==typingText.full)stopTyping();}}>
            {(sayingBye?[{role:"assistant",text:sayingBye}]:msgs).map((m,i,arr)=><div key={i} style={{color:m.role==="assistant"?"#e8d070":"#5b8dd9",fontSize:10,lineHeight:"17px",marginBottom:6,wordBreak:"break-word",textAlign:"left"}}><span style={{color:m.role==="assistant"?(talkingTo==="left"?"#b04040":"#4060b0"):"#70e870"}}>{m.role==="assistant"?(talkingTo==="left"?"GUARD 1":"GUARD 2"):"ANDREW"}:</span> {(typingText&&i===arr.length-1&&m.role==="assistant"&&typingText.current!==typingText.full)?typingText.current:m.text}</div>)}
            {loading&&!sayingBye&&<div style={{color:"#6a6a8a",fontSize:11}}>The guard ponders...</div>}
            <div ref={chatEndRef}/>
          </div>
          {!sayingBye&&!hardModeAnswered&&<div style={{display:"flex",gap:6}}>
            ...
            <button onClick={sendMessage} style={{background:"#e8d070",border:"none",borderRadius:"50%",fontFamily:FONT,fontSize:16,width:48,height:48,cursor:"pointer",color:"#0e0c14",display:"flex",alignItems:"center",justifyContent:"center"}}>▲</button>
          </div>}
          {hardModeAnswered&&!sayingBye&&(
            <button onClick={()=>{setChatOpen(false);guardFadingRef.current=true;try{const s=new Tone.Synth({oscillator:{type:"sawtooth"},envelope:{attack:0.3,decay:1.5,sustain:0,release:0.5},volume:-14}).toDestination();s.triggerAttackRelease("D2","2n");setTimeout(()=>{const s2=new Tone.Synth({oscillator:{type:"sine"},envelope:{attack:0.2,decay:1,sustain:0,release:0.3},volume:-16}).toDestination();s2.triggerAttackRelease("Ab1","2n");},400);}catch{}}} style={{marginTop:6,background:"#2a1020",border:"2px solid #cc4444",borderRadius:4,color:"#ff6644",fontFamily:FONT,fontSize:10,padding:"12px 8px",cursor:"pointer",width:"100%",minHeight:44}}>NEXT...</button>
          )}
...
function drawEntryHall(ctx) {
  ...
  ctx.font="10px "+FONT;ctx.fillStyle="#e8d070";ctx.textAlign="center";ctx.fillText("EXIT",10*TILE+TILE/2,(L2_ROWS-1)*TILE-5);
  ctx.textAlign="left";
  ...
  // Door labels — with dark background for readability
  ctx.font="10px "+FONT;ctx.fillStyle="#cc8830";ctx.textAlign="left";ctx.fillText("BATH",4,7*TILE-5);ctx.textAlign="right";ctx.fillText("GIFT",(L2_COLS-1)*TILE+TILE-4,7*TILE-5);
  ctx.textAlign="left";
  // Room label
  ctx.fillStyle="rgba(0,0,0,0.6)";ctx.fillRect(L2_W/2-180,4,360,16);ctx.fillStyle="#f0e8d0";ctx.font="11px "+FONT;ctx.textAlign="center";ctx.fillText("ASHWORTH GALLERY  ENTRY HALL",L2_W/2,16);
  ctx.textAlign="left";
...
function drawBathroom(ctx) {
  ...
  ctx.fillStyle="#e8d070";ctx.fillRect((L2_COLS-1)*TILE+6,7*TILE+14,4,4);
  // Door label
  ctx.font="10px "+FONT;ctx.fillStyle="#e8d070";ctx.textAlign="right";ctx.fillText("EXIT",(L2_COLS-1)*TILE+TILE-4,7*TILE-5);
  ctx.textAlign="left";
  // Room label
  ctx.font="11px "+FONT;ctx.textAlign="center";ctx.fillStyle="#3a6a8a";ctx.fillText("BATHROOM",L2_W/2,TILE+10);
  ctx.textAlign="left";
  // Sinks
...
function drawGiftShop(ctx) {
  ...
  ctx.fillStyle="#e8d070";ctx.fillRect(22,7*TILE+14,4,4);
  // Door label
  ctx.font="10px "+FONT;ctx.fillStyle="#e8d070";ctx.textAlign="left";ctx.fillText("EXIT",4,7*TILE-5);
  ctx.textAlign="left";
  // Right wall (solid)
  ...
  // Room label
  ctx.font="11px "+FONT;ctx.textAlign="center";ctx.fillStyle="#6a4a2a";ctx.fillText("GIFT SHOP",L2_W/2,TILE+10);
  ctx.textAlign="left";
  // Counter
...
function drawCloseupNotebook(ctx){
  ctx.fillStyle="#1a1420";ctx.fillRect(0,0,128,128);
  // Leather cover
  ctx.fillStyle="#5a3a1a";ctx.fillRect(16,10,96,108);
  ctx.fillStyle="#6b4a2a";ctx.fillRect(20,14,88,100);
  ctx.fillStyle="#7a5a3a";ctx.fillRect(24,18,80,92);
  // Spine
  ctx.fillStyle="#4a2a0a";ctx.fillRect(16,10,8,108);
  ctx.fillStyle="#3a1a00";ctx.fillRect(18,14,4,100);
  // Stitching on spine
  ctx.fillStyle="#8b7a5a";
  for(let i=0;i<8;i++){ctx.fillRect(19,20+i*12,2,4);}
  // Cover texture lines
  ctx.fillStyle="#6b4a2a";
  ctx.fillRect(30,24,74,1);ctx.fillRect(30,106,74,1);
  // "FORENSICS" text on cover
  ctx.fillStyle="#e8d070";ctx.font="8px "+FONT;ctx.textAlign="center";
  ctx.fillText("FORENSICS",68,42);
  // Badge/emblem
  ctx.fillStyle="#c8a040";ctx.fillRect(52,48,32,24);
  ctx.fillStyle="#a08030";ctx.fillRect(54,50,28,20);
  ctx.fillStyle="#e8d070";ctx.fillRect(60,54,16,12);
  ctx.fillStyle="#c8a040";ctx.fillRect(64,56,8,8);
  // Grid lines sketched on lower half
  ctx.fillStyle="#9a8a6a";
  for(let i=0;i<4;i++){ctx.fillRect(34,74+i*8,64,1);}
  for(let i=0;i<4;i++){ctx.fillRect(34+i*16,74,1,24);}
  // Pen
  ctx.fillStyle="#1a1a3a";ctx.fillRect(90,60,4,48);
  ctx.fillStyle="#aaa";ctx.fillRect(90,56,4,6);
  ctx.fillStyle="#cc3030";ctx.fillRect(91,62,2,4);
  // Pen clip
  ctx.fillStyle="#ccc";ctx.fillRect(94,64,3,8);
  // Page edges visible on right
  ctx.fillStyle="#f0ece0";ctx.fillRect(104,18,4,88);
  ctx.fillStyle="#e8e4d8";ctx.fillRect(106,20,2,84);
  ctx.textAlign="left";
}
...
function Level2({ onWin, onRestart, muted, setMuted, muteBtn, startMusicForLevel, apiKey, model }) {
  ...
  const cutsceneTimerRef=useRef(36);
  const hasNotebookRef=useRef(false);
  ...
  useEffect(()=>{if(chatEndRef.current)chatEndRef.current.parentElement.scrollTop=chatEndRef.current.parentElement.scrollHeight;},[messages]);
    ...
    const loop=()=>{
      ...
      if(cutsceneRef.current&&curRoom==="hall"){
        ...
        if(cutsceneRef.current==="pause"){
          ...
          if(cutsceneTimerRef.current<=0){cutsceneRef.current="alert";cutsceneTimerRef.current=55;try{const alertSynth=new Tone.Synth({oscillator:{type:"square"},envelope:{attack:0.01,decay:0.1,sustain:0,release:0.1},volume:-8}).toDestination();alertSynth.triggerAttackRelease("C5","8n");setTimeout(()=>alertSynth.triggerAttackRelease("E5","8n"),100);}catch{}}
      ...
      if(!hasNotebookRef.current&&cr==="giftshop"){
        ...
        ctx.fillStyle="#9a8a6a";
        for(let i=0;i<3;i++){ctx.fillRect(nbx+12,nby+12+i*4,8,1);}
        ctx.fillStyle="#e8d070";ctx.font="3px "+FONT;ctx.textAlign="center";
        ctx.fillText("F",nbx+16,nby+11);ctx.textAlign="left";
      }
      activeNpcs.forEach(npc=>{
        ...
        ctx.font="11px "+FONT;ctx.textAlign="center";ctx.fillStyle=npc.color;ctx.fillText(npc.label,drawX+16,drawY-8);ctx.textAlign="left";
      });
      // Draw "!" alert above detective during alert phase
      if(cutsceneRef.current==="alert"){
        const lp=leadPosRef.current;
        ctx.fillStyle="#fff";ctx.fillRect(lp.x+4,lp.y-44,24,28);
        ctx.strokeStyle="#000";ctx.lineWidth=2;ctx.strokeRect(lp.x+4,lp.y-44,24,28);ctx.lineWidth=1;
        ctx.fillStyle="#cc0000";ctx.font="18px "+FONT;ctx.textAlign="center";
        ctx.fillText("!",lp.x+16,lp.y-21);ctx.textAlign="left";
      }
      drawPlayer(ctx,p.x,p.y,p.dir,p.frame);
...
Keep answers under 2-3 sentences. 8-bit RPG style.`;
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",fontFamily:FONT,background:"#1a1820",minHeight:"100vh",padding:"12px 8px 190px",boxSizing:"border-box",position:"relative"}} onClick={startMusicForLevel}>
      {muteBtn}
      <div style={{color:"#e8d070",fontSize:13,marginBottom:4,textShadow:"2px 2px 0 #3a2a0a",letterSpacing:3}}>MURDER AT THE ASHWORTH GALLERY</div>
      <div style={{color:"#8a8a7a",fontSize:9,marginBottom:4,textAlign:"center",lineHeight:"16px"}}>Talk to suspects. Gather clues. Make your accusation to the detective.</div>
      <div style={{color:"#aaa088",fontSize:10,marginBottom:8,textAlign:"center"}}>📍 {roomLabel}</div>
      <div style={{background:"#28242a",border:"3px solid #5a5040",borderRadius:6,padding:6,boxShadow:"0 0 20px rgba(120,100,60,0.3)",width:"100%",maxWidth:L2_W+12,boxSizing:"border-box"}}>
        {/* Notebook replaces map, or map shows */}
      {notebook.open&&(
        <div style={{background:"#1a1420",borderRadius:4,padding:10,overflowX:"auto",overflowY:"auto",maxHeight:L2_H,width:"100%",maxWidth:L2_W+12,boxSizing:"border-box"}}>
          <div style={{color:"#e8d070",fontSize:9,marginBottom:12,textAlign:"center",letterSpacing:2}}>─── DETECTIVE'S NOTEBOOK ───</div>
          <div style={{overflowX:"auto",marginBottom:12,display:"flex",justifyContent:"center"}}>
            <table style={{borderCollapse:"collapse",fontFamily:FONT}}>
              <thead><tr>
                ...
                {WEAP_LABELS.map((w,i)=><td key={i} style={{width:NB_CELL,textAlign:"center",color:"#e8d070",fontSize:9,padding:"2px 0",fontFamily:FONT}}>{w}</td>)}
                <td style={{width:8}}/>
                {ROOM_LABELS.map((r,i)=><td key={i} style={{width:NB_CELL,textAlign:"center",color:"#e8d070",fontSize:9,padding:"2px 0",fontFamily:FONT}}>{r}</td>)}
              </tr></thead>
              <tbody>
                {[0,1,2].map(r=>(
                  <tr key={"s"+r}>
                    <td style={{color:SUSP_COLORS[r],fontSize:9,paddingRight:4,textAlign:"right",fontFamily:FONT}}>{SUSP_LABELS[r]}</td>
                    {[0,1,2].map(c=><td key={c} onClick={()=>toggleCell("suspectWeapon",r,c)} style={{width:NB_CELL,height:NB_CELL,border:"1px solid #3a3060",background:"#1a1420",textAlign:"center",cursor:"pointer",fontSize:16,color:cellColor(notebook.suspectWeapon[r][c]),fontFamily:"sans-serif",userSelect:"none"}}>{cellDisp(notebook.suspectWeapon[r][c])}</td>)}
                    <td style={{width:8,borderLeft:"2px solid #5a4a30"}}/>
                    {[0,1,2].map(c=><td key={c} onClick={()=>toggleCell("suspectRoom",r,c)} style={{width:NB_CELL,height:NB_CELL,border:"1px solid #3a3060",background:"#1a1420",textAlign:"center",cursor:"pointer",fontSize:16,color:cellColor(notebook.suspectRoom[r][c]),fontFamily:"sans-serif",userSelect:"none"}}>{cellDisp(notebook.suspectRoom[r][c])}</td>)}
                  </tr>
                ))}
          ...
          <div style={{borderTop:"1px solid #3a3060",paddingTop:10,marginBottom:10}}>
            <div style={{color:"#e8d070",fontSize:9,marginBottom:6,textAlign:"center"}}>─── CLUES & EVIDENCE ───</div>
            {discoveredClues.length===0?(
              <div style={{color:"#8a8a7a",fontSize:10,lineHeight:"18px",fontStyle:"italic"}}>No clues discovered yet.</div>
            ):(
              [
                ...
                <div key={i} style={{color:"#c8b880",fontSize:10,lineHeight:"18px",marginBottom:4}}>• {clue}</div>
              ))
            )}
          </div>
      ...
      {showClues&&!chatOpen&&!notebook.open&&(
        <div style={{width:L2_W+12,maxWidth:"100%",background:"#1a1420",border:"2px solid #5a4a30",borderRadius:6,marginTop:6,padding:10,boxSizing:"border-box"}}>
          ...
          {discoveredClues.length === 0 ? (
            <div style={{color:"#8a8a7a",fontSize:10,lineHeight:"18px",fontStyle:"italic"}}>No clues discovered yet. Examine objects and talk to people.</div>
          ) : (
            [
              ...discoveredClues.filter(c=>c.includes("statement:")),
              ...discoveredClues.filter(c=>c.includes("thread")||c.includes("Security footage")),
      ...
      {chatOpen&&ti&&(
        <div style={{width:"100%",maxWidth:500,background:"#1a1420",border:"3px solid "+ti.color,borderRadius:6,marginTop:6,padding:10,boxSizing:"border-box"}}>
          ...
          <div style={{maxHeight:120,overflowY:"auto",marginBottom:8,WebkitOverflowScrolling:"touch",cursor:typingText&&typingText.current!==typingText.full?"pointer":"default"}} onClick={()=>{if(typingText&&typingText.current!==typingText.full)stopTyping();}}>
            {(sayingBye?[{role:"assistant",text:sayingBye}]:curMsgs).map((m,i,arr)=><div key={i} style={{color:m.role==="assistant"?"#e8d070":"#5b8dd9",fontSize:10,lineHeight:"17px",marginBottom:6,wordBreak:"break-word",textAlign:"left"}}><span style={{color:m.role==="assistant"?ti.color:"#70e870"}}>{m.role==="assistant"?ti.name:"ANDREW"}:</span> {(typingText&&i===arr.length-1&&m.role==="assistant"&&typingText.current!==typingText.full)?typingText.current:m.text}</div>)}
...
export default function GameMobile() {
  ...
  if (level === null) return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100vh",background:"#1a1428",color:"#e8d070",fontFamily:FONT,padding:20,textAlign:"center"}}>
      ...
      <button onClick={()=>setLevel("1hard")} style={{width:280,padding:"14px 20px",fontSize:10,fontFamily:FONT,cursor:"pointer",background:"#2a2040",border:"2px solid #3a3060",borderRadius:8,color:"#e8d070",marginBottom:12,minHeight:48,textAlign:"left"}}>
        <span style={{color:"#ff6644"}}>Level 1 (Hard):</span> The Two Guards<br/><span style={{fontSize:7,color:"#8a7a6a",marginTop:4,display:"block"}}>One question only — both guards vanish after</span>
      </button>
